...
class ChatView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.history = cache.get(f'conversation_history_{user_id}')
        self.user_conversation = chroma_client.get_or_create_collection(name=f'conversation_history_user_{user_id}')

        if request.method == 'GET':
            docs = self.user_conversation.get()['documents'][-50:]

            if docs != self.history:
                cache.set(f'conversation_history_{user_id}', docs, timeout=None)
                self.history = docs
